  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative flex md:hidden items-center justify-center w-full">
          <CarouselButton direction="left" onClick={() => scroll('left')} disabled={activeIndex === 0} />
          <TestimonialCard testimonial={testimonials[activeIndex]} />
          <CarouselButton direction="right" onClick={() => scroll('right')} disabled={activeIndex === testimonials.length - 1} />
        </div>
...
const TestimonialCard = ({ testimonial }) => {
  return (
    <article
      className="bg-secondary rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 w-[85vw] sm:w-[260px] md:w-auto flex-shrink-0"
      aria-label={`Testimonial from ${testimonial.name}`}
    >
      <div className="relative pt-[177.78%] overflow-hidden">
        <iframe
...
const CarouselButton = ({ direction, onClick, disabled }) => (
  <button
    className={`absolute top-1/2 -translate-y-1/2 z-10 w-10 h-10 md:w-12 md:h-12 glass rounded-full flex items-center justify-center text-white text-2xl font-bold hover:bg-white/20 hover:shadow-lg transition-transform duration-300 transform hover:scale-110 focus:outline-none ${
      direction === 'left' ? 'left-2' : 'right-2'
    } disabled:opacity-30 disabled:pointer-events-none`}
    onClick={onClick}
    disabled={disabled}
