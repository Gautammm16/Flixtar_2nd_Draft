...
const Testimonials = () => {
  ...
  return (
    <section className="py-20 bg-primary" id="testimonials-carousel">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <header>
        ...
        >
          {testimonials.map((testimonial, index) => (
            <SwiperSlide key={index}>
              <article
                className="bg-white shadow-xl rounded-2xl p-6 md:p-8 mx-2 sm:mx-4 transition-transform duration-300 hover:scale-[1.03]"
                data-aos="fade-up"
                data-aos-delay={index * 100}
                aria-label={`Testimonial from ${testimonial.name}`}
              >
                <div className="mb-6 w-20 h-20 mx-auto overflow-hidden rounded-full border-2 border-primary">
                  <img
                    src={testimonial.img}
                    alt={`Photo of ${testimonial.name}`}
                    className="w-full h-full object-cover object-center"
